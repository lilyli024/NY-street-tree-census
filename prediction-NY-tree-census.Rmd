...
knn_grid <- grid_regular(degree(range = c(1, 10)), levels = 10)
...
best_deg_knn <- select_by_one_std_err(tune_log, degree, metric = "accuracy")
final_knn <- finalize_workflow(knn_wkflow, best_deg_knn)

knn_fit <- fit(final_knn, train)

predict(knn_fit, new_data = test, type = "class") %>%
  bind_cols(select(test, health)) %>%
  accuracy(truth=health, estimate= .pred_class) # 0.6869773	

results <- augment(knn_fit, new_data = test)
results %>% roc_auc(truth = health, .pred_Poor:.pred_Good) # 0.6465457	
results %>% roc_curve(truth = health, .pred_Poor:.pred_Good) %>% autoplot()
# stronger at distinguishing classifications "Good"


model_metrics <- data.frame("Model"=c("KNN"), "Accuracy"=c(0.6869773), "ROC_AUC"=c(0.6465457))
```
...
best_deg_boost <- select_by_one_std_err(tune_boost, degree, metric = "accuracy")
final_boost <- finalize_workflow(boost_wkflow, best_deg_boost)

boost_tree_fit <- fit(final_boost, train)

boost_tree_pred <- predict(boost_tree_fit, new_data = test, type = "class") %>%
  bind_cols(select(test, health)) %>%
  accuracy(truth=health, estimate= .pred_class) # 0.7461171	

boost_tree_results <- augment(boost_tree_fit, new_data = test) 
boost_tree_results %>% roc_auc(truth = health, estimate= .pred_Poor:.pred_Good) # 0.693939		
...
model_metrics[2,] <- c("Boosted Trees", 0.7461171, 0.693939)
```
...
best_deg_rf <- select_by_one_std_err(tune_rf, degree, metric = "accuracy")
final_rf <- finalize_workflow(rf_wkflow, best_deg_rf)

rf_fit <- fit(final_rf, train)

predict(rf_fit, new_data = test, type = "class") %>%
  bind_cols(select(test, health)) %>%
  accuracy(truth=health, estimate= .pred_class) # 0.0.744325	

rf_results <- augment(rf_fit, new_data = test) 
rf_results %>% roc_auc(truth = health, estimate= .pred_Poor:.pred_Good) # 	

model_metrics[3,] <- c("Random Forests", 0.744325, 0.5)
```
