...
ggplot() +
  geom_polygon(aes(x = long, y = lat, group = group),fill = "white", colour = "grey50", data = nyc) +
  xlab('Longitude') + 
  ylab('Latitude') + 
  stat_density2d(aes(x = longitude, y = latitude, fill = status), alpha = .5,
                 h = .02, n = 200,
                 geom = "polygon", data = clean  %>% head(10000)) + 
  xlim(-74.25, -73.70) + ylim(40.50, 40.91)

```

```{r}
borough_data <- raw_data %>% 
  mutate(stat_health = ifelse(status == "Alive", as.character(health), as.character(status))) %>%
  mutate(stat_health = factor(stat_health, levels = c("Dead", "Poor", "Fair", "Good"))) %>%
  select(borough, stat_health) %>% na.omit()

ggplot(borough_data , aes(y = reorder(borough, borough, function(x) length(x)), fill = stat_health)) +
  geom_bar(position = 'stack') +
  labs(y = "Borough")
```
